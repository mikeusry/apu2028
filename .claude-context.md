...
## School / Notion (parked from the leftover APU-Training checkout, 2026-01-09)

Token is in `~/.claude/settings.json` → `mcpServers.notion.env.NOTION_TOKEN` (not `settings.local.json`). Notion MCP writes are broken; curl the API. All times EST (`-05:00`).

| Database | ID (query) | Parent ID (create) |
|----------|------------|-------------------|
| Courses | `2e3cee77-0ea4-80c9-84b5-000b44d960c9` | `2e3cee77-0ea4-80d0-97be-ca415fb1df73` |
| School Calendar | `0dfce738-aa9a-467e-befb-bc32c8665005` | `83857c00-71e8-4bb8-8d43-4fa0b58f19cd` |
| School Hub Page | `2cbcee77-0ea4-8125-965f-dfd971fcb9cf` | — |
| Softball Schools | `2bacee77-0ea4-807d-b261-000bd48d18ee` | — |
| College Contacts | `2a2cee77-0ea4-809f-b361-000bf2a8185a` | — |
| Recruiting Actions | `2d2cee77-0ea4-8131-9e02-f80f5612dce7` | — |
| Training Calendar | `5c95168a-118c-4f9c-bd0b-488016d1fdb1` | — |

**Spring 2026 live classes** (self-paced: Apologetics, Girls PE II, Academic & Career Success)

| Course | Day | Time | Page ID |
|--------|-----|------|---------|
| Geometry | Wed | 9:00–10:00 AM | `2e3cee77-0ea4-81e4-b72d-f96a786f4063` |
| World Geography | Tue | 10:10–11:10 AM | `2e3cee77-0ea4-81df-84b0-c646310ce2a5` |
| Spanish II | Fri | 10:10–11:10 AM | `2e3cee77-0ea4-81f5-a20b-f468c33d6d82` |
| Chemistry | Wed | 11:20 AM–12:20 PM | `2e3cee77-0ea4-816b-9903-dc443a0850cf` |
| English 10 | Mon | 11:20 AM–12:20 PM | `2e3cee77-0ea4-813e-8d8d-cf0866d0fad2` |
| Apologetics | — | self-paced | `2e3cee77-0ea4-81c9-bbd4-e185c65c31cf` |
| Girls PE II | — | self-paced | `2e3cee77-0ea4-8184-9b56-d0db688ca2ac` |
| Academic & Career Success | — | self-paced | `2e3cee77-0ea4-815e-a9fc-d44e69919631` |
...
1. **School Hub Setup in Notion**
   - Ayn starting LUOA (Liberty University Online Academy) Spring 2026
   - 8 courses starting Jan 12, 2026
   - Created Courses database with 14 properties
   - All 8 courses populated with full metadata

2. **Courses Database Created**
   - **Database ID:** `2e3cee77-0ea4-80c9-84b5-000b44d960c9` (view)
   - **Parent Database ID:** `2e3cee77-0ea4-80d0-97be-ca415fb1df73` (for API writes)
   - **School Hub Page:** `2cbcee77-0ea4-8125-965f-dfd971fcb9cf`

   Properties: Name, Code, Category, Semester, NCAA Core, Status, Current Grade, Weekly Hours, Live Class Day, Live Class Time, Instructor, Syllabus URL, Start Date, End Date

3. **Spring 2026 Courses (8 total)**
   | Course | Code | Category | NCAA Core |
   |--------|------|----------|-----------|
   | English 10 | LAN1002 | English | Yes |
   | Geometry | MAT1002 | Math | Yes |
   | Chemistry | SCI1102 | Science | Yes |
   | World Geography | HIS0952 | History | Yes |
   | Spanish II | LAN2302 | Foreign Language | Yes |
   | Apologetics | BIB1001 | Bible/Religion | No |
   | Girls PE II | HPEG250 | Health/PE | No |
   | Academic & Career Success | APP2050 | Elective | No |

4. **Syllabus Import System Created**
   - `school/syllabus-template.csv` - Structured format for syllabus data
   - `school/SYLLABUS-IMPORT.md` - Full workflow documentation
   - `school/spring-2026/` - Directory for course syllabi

### CRITICAL: Notion API Workaround

**Problem:** The Notion MCP tool (`@notionhq/notion-mcp-server`) has a bug where it stringifies the `parent` parameter instead of passing it as an object. This breaks page creation in databases.

**Solution:** Use curl directly to the Notion API:
...
**Token Location:** `~/.claude/settings.local.json` → `mcpServers.notion.env.NOTION_TOKEN`

**Key Insight:** When creating pages in a Notion database:
- Use the **parent database_id** from the database's `parent` field, NOT the database's own ID
- The database ID (`2e3cee77-0ea4-80c9-84b5-000b44d960c9`) is for querying/updating
- The parent ID (`2e3cee77-0ea4-80d0-97be-ca415fb1df73`) is for creating new pages

### Architecture Decisions

1. **School Management via Notion** - School Hub as central interface
2. **Courses Database** - One entry per course with syllabus metadata
3. **School Calendar Database** - Individual assignments/deadlines (already existed)
4. **Potential:** Sub-databases inside each course for detailed syllabus breakdown (TBD when syllabi arrive)

### Schedule Collision Identified

Training Calendar (Jan 12+) conflicts with school:
- Plyos with Maggie: Tue/Fri @ 1:30 PM (splits school day)
- CCHS Track: Mon-Thu @ 3:45 PM (limits afternoon school)
- D1 Lifting, Michael Clarke hitting, etc.

School blocks needed: 8 AM - 1:30 PM, 5 PM+ (after Track)

### Git State
- New: `school/` directory (uncommitted)
- Ready to commit

### Next Steps
1. Wait for LUOA Canvas access (Jan 12)
2. Screenshot/PDF syllabi → Claude extracts structured data
3. Populate School Calendar with deadlines
4. Build unified calendar view (Training + School + Family)
5. Consider sub-databases inside each course page for detailed tracking
...
1. **Forked SaaS Product to New Repo**
   - Created `/Users/mikeusry/CODING/program-match`
   - Migrated all product/outreach docs for softball discovery SaaS
   - Clean separation: apu2028 = Ayn's site, program-match = SaaS product

2. **program-match Repo Structure**
   ```
   program-match/
   ├── README.md              # Project overview, links to apu2028
   ├── ORIGIN.md              # Full backstory and lineage
   ├── .claude-context.md     # AI session context
   └── docs/
       ├── product/           # Product spec, validation feedback
       ├── outreach/          # Email templates
       └── planning/          # Planning phase docs (empty, ready)
   ```

3. **Documents Migrated**
   - `softball-discovery-saas.md` (656-line product spec)
   - `validation-feedback.md` (Caitlin + Joe responses)
   - All outreach templates (parent, coach, advisor)

4. **What Stays in apu2028**
   - Ayn's recruiting site code
   - Coach visits dashboard
   - Personal recruiting strategy docs
   - Notion CRM integration (in progress)

### Git State
- apu2028: Clean, all changes committed
- program-match: Initial commit `f370456`

### Next Steps for apu2028
- Fix video tracking (play events not detected)
- Fix 30s+ engagement timer
- Complete Notion integration (Phase 2)

### Next Steps for program-match
- Work happens in that repo now
- Complete validation outreach
- Design data model
- Plan MVP scope

---

## Previous Session: December 28, 2025 (Night)
